...
class ScannerEngine {
     ...
     */
    static checkRSIOverbought(candles, threshold = 70) {
        const closes = candles.map(c => c.close);
        const rsiValues = indicator_engine_1.IndicatorEngine.getRSI(14, closes);
     ...
     */
    static checkVolumeBreakout(candles, multiplier = 2.5, period = 20) {
     ...
     */
    static checkUptrend(candles) {
        const closes = candles.map(c => c.close);
        const ema20 = indicator_engine_1.IndicatorEngine.getEMA(20, closes);
        const ema50 = indicator_engine_1.IndicatorEngine.getEMA(50, closes);
     ...
     */
    static runSwingScanner(symbol, dailyCandles) {
        return {
            symbol,
    ...
    static calculateMomentumScore(candles) {
        let score = 0;
        if (this.checkUptrend(candles))
            score += 40;
        if (this.checkVolumeBreakout(candles))
            score += 30;
        const closes = candles.map(c => c.close);
        const rsi = indicator_engine_1.IndicatorEngine.getRSI(14, closes);
        if (rsi.length > 0) {
            const latestRsi = rsi[rsi.length - 1];
            if (latestRsi > 50 && latestRsi < 75)
