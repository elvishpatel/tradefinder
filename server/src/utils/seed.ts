...
export async function seedDatabase() {
  ...
  try {
    ...
    for (const sec of SECTORS_SEED) {
      ...
      if (error) {
        logger.error(`Error querying sector ${sec.name}: ${error.message}`);
        continue;
      }
      ...
      if (!data) {
        ...
        if (insErr) {
          logger.error(`Error inserting sector ${sec.name}: ${insErr.message}`);
        } else {
          logger.info(`Seeded sector: ${sec.displayName}`);
        }
      }
    }

    // Load sectors lookup mapping (name -> id)
    const { data: sectorsDb, error: secErr } = await supabase
      .from('sectors')
      .select('id, name');
    ...
    for (const stock of STOCKS_SEED) {
      ...
      if (!sectorId) {
        logger.warn(`No sector found for stock ${stock.symbol} under ${stock.sectorName}`);
        continue;
      }
      ...
      if (error) {
        logger.error(`Error querying stock ${stock.symbol}: ${error.message}`);
        continue;
      }
      ...
      if (!data) {
        ...
        if (insErr) {
          logger.error(`Error inserting stock ${stock.symbol}: ${insErr.message}`);
        } else {
          logger.info(`Seeded stock: ${stock.symbol} (${stock.companyName})`);
        }
      }
    }
